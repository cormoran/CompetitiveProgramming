...
(Setting& setting, BattleField& field, int id) {
    ...
    int pipeToAI[2];
    if (pipe(pipeToAI) < 0) {
        throw ErrorReport("Failed to create a pipe: "+ to_string(errno));
    }
    int pipeFromAI[2];
    ...
    processId = fork();
    if (processId == -1) {
        throw ErrorReport("Failed to fork an AI process: " + to_string(errno));
    }
    if (processId == 0) {
        // Child process
        // stdin from the pipe
        ...
        if (fai == nullptr) {
            throw ErrorReport("Failed to open pipe to AI: "+to_string(errno));
        }
        fromAI = SamuraiScanner(fai);
        toAI = fdopen(pipeToAI[1], "w");
        if (toAI == nullptr) {
            throw ErrorReport("Failed to open pipe from AI: "+to_string(errno));
        }
    }
}
...
void GameState::sendTurnInfo(FILE* out, int side, int weapon) {
    ...
    if (ss.alive) {
        ...
        vector<bool> visible(width * height, false);
        for (int w = 0; w != 3; w++) {
            int who = 3*side+w;
        ...
        fprintf(log, "# Actions\n");
        {
            ...
            while (!ss.actions.empty()) {
                ...
                if (action < 0 || action > 10) {
                    turnComments += "Invalid action specified: " + to_string(action);
                    goto FINISH;
                } else {
