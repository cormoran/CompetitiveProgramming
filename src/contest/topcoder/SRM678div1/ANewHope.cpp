...
class ANewHope {
  public:
    int count( vector <int> firstWeek, vector <int> lastWeek, int D ) {
        ...
        vector<int> lastWeek_index(N+1);
        rep(i,N){
            lastWeek_index[lastWeek[i]] = i;
        }

        int ans = 1;
        rep(i,N){
            int src = i;
            int dest = lastWeek_index[firstWeek[i]];
            if( src <= dest ) continue;
            int forwardable = N-D;
            ans = max(ans, (int)ceil((double)(src-dest)/forwardable) +1);
        }
        return ans;
    }
};
